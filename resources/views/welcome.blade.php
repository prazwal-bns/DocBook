            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    @if (Auth::check())
                        ...
                        <a href="{{ route('patient.dashboard') }}" class="font-medium text-white hover:text-yellow-400 {{ request()->routeIs('patient.dashboard') ? 'text-yellow-400' : '' }}">Dashboard</a>
                    @else
                        <!-- If user is not logged in, show Login and Register -->
                        <a href="{{ route('login') }}" class="font-medium text-white hover:text-yellow-400 {{ request()->routeIs('login') ? 'text-yellow-400' : '' }}">Login</a>
                        <a href="{{ route('register') }}" class="px-4 py-2 font-medium text-white transition duration-200 bg-indigo-500 rounded-md hover:bg-blue-600 {{ request()->routeIs('register') ? 'text-yellow-400' : '' }}">Register</a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="px-4 py-16 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="lg:grid lg:grid-cols-12 lg:gap-8 lg:items-center">
                <div class="sm:text-center md:max-w-2xl md:mx-auto lg:col-span-6 lg:text-left">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('register') }}" class="px-6 py-3 text-white bg-blue-600 rounded-md shadow hover:bg-blue-700">
                            Register Here
                        </a>
                    </div>
                </div>
    ...
    <section id="contact" class="py-12 bg-gradient-to-r from-indigo-50 to-indigo-100">
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="max-w-2xl mx-auto mt-8">
                <form action="{{ route('send.message') }}" method="POST" class="p-8 space-y-6 shadow-lg bg-blue-20 rounded-xl">
                    @csrf
                    <div>
