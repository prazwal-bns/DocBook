...
@section('content')
    <div class="p-6 bg-white rounded-lg shadow-md ">
        ...
        @if (session('success'))
            <div
                class="flex items-center px-6 py-4 mb-6 text-green-800 bg-green-100 border border-green-300 rounded-md shadow-lg">
                ...
                <tbody>
                    @foreach ($appointmentData as $appointment)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2">
                                {{ \Carbon\Carbon::parse($appointment->appointment_date)->format('F j, Y') }}</td>
                            <td class="px-4 py-2">{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('l') }}
                            </td>
                            <td class="px-4 py-2">{{ \Carbon\Carbon::parse($appointment->start_time)->format('h:i A') }}
                            </td>
                            ...
                            <td class="px-4 py-2 text-center">
                                @if ($appointment->doctor->status == 'not_available')
                                    <p class="font-bold text-red-700">Dr. {{ $appointment->doctor->user->name }} is
                                        currently
                                        unavailable.</p>
                                @else
                                    <div class="flex space-x-4">
                                        <a href="{{ route('view.appointment.details', $appointment->id) }}"
                                            class="inline-block px-4 py-2 ml-4 text-sm font-semibold text-white bg-green-600 rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
                                            View Appointment
                                        </a>
                                        @if ($appointment->status === 'pending')
                                            ...
                                            <form action="{{ route('delete.myAppoinment', $appointment->id) }}"
                                                method="POST"
                                        ...
                                        @if ($appointment->status === 'completed')
                                            <a href="{{ route('view.doctor.review', $appointment->id) }}"
                                                class="inline-block px-4 py-2 ml-4 text-sm font-semibold text-white bg-gray-600 rounded-lg shadow-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50">
                            ...
                            <td class="px-4 py-2">
                                @if ($appointment->payment->payment_status !== 'paid')
                                    <div class="flex items-center space-x-1">
                                        ...
                                        {{-- <a href="{{ route('stripe.payment', ['appointmentId' => $appointment->id]) }}" class="inline-block px-4 py-2 ml-4 text-sm font-semibold text-white bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                            >Pay via Stripe</a> --}}
                                        
                                        @php
                                            $encryptedId = Crypt::encrypt($appointment->id);
                                        @endphp

                                        <a href="{{ route('stripe.payment', ['appointmentId' => $encryptedId]) }}" 
                                        class="inline-block px-4 py-2 ml-4 text-sm font-semibold text-white bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                                        Pay via Stripe
                                        </a>
